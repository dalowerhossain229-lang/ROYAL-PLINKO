...
// 🚀 [মেগা ৫০০ ওডস ম্যাট্রিক্স চাবি]: কিংস কমান্ড অনুযায়ী দুপাশে ৫০০ লক করা ৮টি স্লটের অফিশিয়াল মাল্টিপ্লায়ার তালিকা
const plinkoMultipliers = [500, 2.0, 0.5, 0.2, 0.2, 0.5, 2.0, 500];
...
// 🛫 ২. প্লিংকো কোর বল ড্রপিং রাউট (POST Route - ৯৫% RTP গাণিতিক বর্ম কঠোর লক ভাই ভাই!)
app.post('/api/plinko-drop', async (req, res) => {
    ...
    try {
        ...
        while (isLoopActive && loopSafety < 200) {
            ...
            // ৮টি পেগের সারি ভেদ করে বল ডানে-বামে যাওয়ার ওরিজিনাল ক্যানভাস পাথ জেনারেটর (০ থেকে ৭ নম্বর স্লট)
            dropPath = [];
            let currentX = 0;
            for (let r = 0; r < 7; r++) {
                let step = (Math.random() < 0.5) ? 0 : 1; // ০ = বামে ড্রপ, ১ = ডানে ড্রপ ভাই ভাই
                dropPath.push(step);
                currentX += step;
            }
            
            finalSlotIdx = currentX; // বলটি শেষমেশ যে স্লট ইনডেক্সে গিয়ে পড়বে
            winMultiplier = plinkoMultipliers[finalSlotIdx];
            ...
            if (adminTriggeredPrize) {
                ...
                // ৫০০ গুণের মেগা জ্যাকপটের চান্স আরটিপি লুপ ট্র্যাকে স্বাভাবিক সিকিউরিটিতে টাইট ০.৫% এ লক ভাই ভাই
                if (winMultiplier === 500 && Math.random() > 0.005) continue;
        ...
        if (winMultiplier > 0) {
            ...
            if (winMultiplier >= 1.0) {
                ...
                // বাজি রিটার্ন ০.২ বা ০.৫ হলে ওরিজিনাল পার্শিয়াল ক্যাশব্যাক রিটার্ন চাবি ভাই ভাই
                dbAction = "win"; 
                dbAmount = winAmount;
            }
        }
...
// রয়্যাল প্লিংকো গেম নিজস্ব কাস্টম ৪০০০ পোর্টে কড়া নিয়নে অন ফায়ার ভাই ভাই!
